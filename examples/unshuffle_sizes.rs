...
fn bench(size: usize) {
    ...
    for i in 0..size { src[i] = (i * 7) as u8; }
    let mut shuffled = vec![0u8; size];
    filters::shuffle(4, &src, &mut shuffled);
    let mut restored = vec![0u8; size];
    for _ in 0..3 { filters::unshuffle(4, &shuffled, &mut restored); }
    let iters = 10000;
    let t = Instant::now();
    for _ in 0..iters {
        filters::unshuffle(4, std::hint::black_box(&shuffled), std::hint::black_box(&mut restored));
    }
    let el = t.elapsed();
    let per = el / iters;
